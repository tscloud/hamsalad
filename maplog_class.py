...
class KmlLog(object):
    ...
    if platform.system() == 'Windows':
        CTYDAT_FILE = 'C:/tc/download/Dropbox/ham/calldata/cty.dat'
    else:
        CTYDAT_FILE = '/home/tscloud/Dropbox/ham/calldata/cty.dat'
    ...
    def __init__(self, fpath_name=None, c=None, q=None):
        ...
        self.qrz = q
        # might (will) need a Ctydat but make it gets made only when we know we need it
        self.ctydat = None
        # might (will) need a HamQTH but it's not passed in -> the others can be used
        #  elsewhere -> not so w/ this guy
        self.hamqth = Hamqth('hamdata_hamqth01', self.fpath)
        self.hamqth.login()
